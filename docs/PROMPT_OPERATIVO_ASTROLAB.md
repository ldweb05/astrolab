...
**Niente PR di default — commit diretto sul branch di lavoro** (deciso 2026-08-19,
dopo prova pratica su PR #1 e #2: il flusso con PR richiede troppi passaggi extra
— autenticazione, verifica mergeability, allineamento locale post-merge, pulizia branch —
rispetto al beneficio per un progetto personale a singolo committente). Il flusso resta:
`git add <file specifici> && git commit -m "..." && git push origin <branch>`, come descritto
sopra. `gh` resta installato e autenticato sul Pi (account `ldweb05`) per un eventuale uso
puntuale, ma non è un passaggio automatico del workflow.
