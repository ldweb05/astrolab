...
<h2>Ricerca Migliori Località RL</h2>
</div>
<!-- ══════════════════════════════════════════════════════════════════
...
<div class="avanzati-group">
    ...
    <select id="filt-grid-search" disabled>
        <option value="no">Disabilitata</option>
    </select>
    <div style="color:#b56;font-size:11px;margin-top:4px">Non ancora disponibile per la ricerca RL</div>
</div>
...
<div><span>RL GMT: </span><b id="info-rs-gmt">—</b></div>
...
<span class="tooltip-wrap">
    <i class="tooltip-icon">i</i>
    <span class="tooltip-box">RS escluse perché presentano Sole/Marte in I/VI/XII RS, ASC RS in I/VI/XII natale, Saturno in X RS o uno stellium in qualsiasi casa RS. Questo è un filtro aggiuntivo di Astrolab, non parte delle 34 regole ufficiali dell'Astrologia Attiva: alle latitudini estreme può escludere risultati che altri software di riferimento mostrano comunque, perché lì gli stellium sono più frequenti per compressione delle case.</span>
</span>
...
function avviaRicerca(espansioneOrbe) {
    ...
    setProgress(0, 'Calcolo momento RL...');
...
g: s.giorno, m: s.mese, a: s.anno,
ora_gmt: s.ora_gmt, lat: s.lat, lon: s.lon,
anno_rs:         document.getElementById('anno-rs').value,
rl_index:        document.getElementById('rl-index').value,
condizione:      document.getElementById('condizione').value,
...
eventoCorrente = new EventSource('api/ricerca_stream_rl_api.php?' + params.toString());
collegaHandlerStandard(eventoCorrente, params);
}
function popolaSelectRLPagina() {
    const s = getSoggetto();
    const sel = document.getElementById('rl-index');
    if (!s) { sel.innerHTML = '<option value="">— Seleziona un soggetto —</option>'; return; }
    const params = new URLSearchParams({
        action: 'lista', soggetto_id: s.id,
        anno_rs: document.getElementById('anno-rs').value,
        lat: s.lat, lon: s.lon,
    });
    fetch('api/rl_api.php?' + params.toString())
        .then(r => r.json())
        .then(data => {
            if (!data.ok) { sel.innerHTML = '<option value="">Errore caricamento RL</option>'; return; }
            const MESI = ['','Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'];
            sel.innerHTML = '';
            data.rl_list.forEach((rl, idx) => {
                const opt = document.createElement('option');
                opt.value = idx;
                opt.textContent = `☽ RL ${idx + 1} — ${rl.giorno} ${MESI[rl.mese] || rl.mese} ${rl.anno}`;
                sel.appendChild(opt);
            });
        })
        .catch(() => { sel.innerHTML = '<option value="">Errore rete</option>'; });
}
// ── ASTRI NELLE CASE ──────────────────────────────────────────────────────
...
eventoCorrente = new EventSource('api/ricerca_stream_rl_api.php?' + prossimiParams.toString());
...
const d = JSON.parse(e.data);
document.getElementById('progress-sub').textContent =
'Deduplicazione ' + d.totale_aeroporti.toLocaleString() + ' aeroporti... RL: ' + d.rl_gmt;
document.getElementById('info-rs-gmt').textContent  = d.rl_gmt;
...
const rlIdx = document.getElementById('rl-index').value;
const rlUrl = 'rl.php?id=' + (sogg?.id||'') +
'&lat_rl=' + r.lat + '&lon_rl=' + r.lon +
'&luogo_rl=' + encodeURIComponent(luogoRs) +
'&anno=' + anno + '&rl_index=' + rlIdx;
const cls     = r.stelline>=5?'stelle-5':r.stelline>=4?'stelle-4':'';
const hasVeti = r.veti && r.veti.length > 0;
...
<td><a href="${rlUrl}" class="btn-usa" target="_blank">☽ Usa</a></td>
