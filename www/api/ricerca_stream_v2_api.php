...
$totaleValutazioniRuleEngine = 0; // diagnostica: numero chiamate RuleEngine::valuta()
    ...
    while ($selezionati !== []) {
        foreach ($selezionati as $aero) {
            ...
            try {
                ...
                // ── E-v2. Calcolo Stelline V2 parallelo ─────────────────────
                ...
                // ── F. Filtro stelline minime sistema attuale ─────────────────
                if ($stellineMin > 0 && $val['stelline'] < $stellineMin) {
                    $processed++;
                    continue;
                }

                // ── F-v2. Filtro pertinenza V2 ────────────────────────────────
                // Nel laboratorio V2 mostriamo solo località che ottengono almeno
                // 1 stella V2 rispetto alla condizione scelta. Questo evita di
                // mostrare RSM/RL che non hanno nulla a che vedere con Salute,
                // Denaro, Amore, Lavoro, Casa, ecc.
                if (($valV2['stelle_totali'] ?? 0) <= 0) {
                    $processed++;
                    continue;
                }
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Ordinamento V2: prima per v2_stelle_totali desc, poi stelline attuali come tiebreaker
    usort($risultati, static function(array $a, array $b): int {
        $cmp = ($b['v2_stelle_totali'] ?? 0) <=> ($a['v2_stelle_totali'] ?? 0);
        if ($cmp !== 0) return $cmp;
        return $b['stelline'] <=> $a['stelline'];
    });
