...
if ($raw === false) {
    echo "✗ ricerca Denaro 2026: impossibile chiamare API\n";
    exit(1);
}

if (!preg_match('/event:\s*done\s+data:\s*(\{.*\})/s', $raw, $m)) {
    echo "✗ ricerca Denaro 2026: evento done non trovato\n";
    exit(1);
}
...
if (!is_array($data)) {
    echo "✗ ricerca Denaro 2026: JSON done non valido\n";
    exit(1);
}
...
if ($totale !== 318) {
    echo "✗ ricerca Denaro 2026: attesi 318 risultati, ottenuti {$totale}\n";
    exit(1);
}

echo "✓ ricerca Denaro 2026: 318 risultati\n";
