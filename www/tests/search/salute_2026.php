...
if (!preg_match('/event:\s*done\s+data:\s*(\{.*\})/s', $raw, $m)) {
    echo "✗ ricerca Salute 2026: evento done non trovato\n";
    exit(1);
}
...
if (!is_array($data)) {
    echo "✗ ricerca Salute 2026: JSON done non valido\n";
    exit(1);
}
...
if ($totale !== 416) {
    echo "✗ ricerca Salute 2026: attesi 416 risultati, ottenuti {$totale}\n";
    exit(1);
}

echo "✓ ricerca Salute 2026: 416 risultati\n";
